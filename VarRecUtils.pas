...
procedure AddConstArray(vAr AArray: TConstArray; const Elements: array of const);
...
var
  I, LIndex: Integer;
begin
  LIndex := Length(AArray);
  SetLength(AArray, Length(AArray) + Length(Elements));
  for I := Low(Elements) to High(Elements) do
    AArray[I+LIndex] := CopyVarRec(Elements[I]);
end;
