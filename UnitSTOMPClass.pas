...
type
  ...
  public
    ...
    constructor CreateWithStr(AUserId, APasswd, AHostIp, APort, ATopic: string;
    ...
    procedure StompSendMsgThread(AMsg: string; ATopic: string = '');
...
begin
  ...
  LPort := StrToIntDef(FHostPort, 0);

  if LPort = 0 then
    LPort := DEFAULT_STOMP_PORT;

  FStomp.Connect(FHostIP,LPort);
...
constructor TpjhSTOMPClass.CreateWithStr(AUserId, APasswd, AHostIp, APort, ATopic: string;
